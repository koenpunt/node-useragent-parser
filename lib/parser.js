...
(function(exports){
  ...
  UserAgentParser = (function(){
    ...
    UserAgentParser.prototype.MatchSpans(self, user_agent_string):
      match_spans = []
    ...
    UserAgentParser.prototype.parse = function(user_agent_string){
      ...
      if( match ){
        if( this.family_replacement ){
          if( this.family_replacement.search(RegExp.quote('$1')) ){
            family = this.family_replacement.replace(RegExp.quote('$1'), match[1]);
          }else{
    ...
    DeviceParser.prototype.parse = function(user_agent_string){
      ...
      if( match ){
        if( this.device_replacement ){
          if( this.device_replacement.search(RegExp.quote('$1')) ){
            device = this.device_replacement.replace(RegExp.quote('$1'), match[1]);
          }else{
  ...
  function parse_user_agent(user_agent_string, js_parse_bits){
     ...
     */
    var i, ua_parser, parsed, family, v1, v2, v3, js_override;
  ...
  function parse_device(user_agent_string, ua_family, os_family){
    ...
    if( (ua_family in MOBILE_USER_AGENT_FAMILIES) ){
      is_mobile = true;
    }else if( (os_family in MOBILE_OS_FAMILIES) ){
      is_mobile = true;
    }else{
      is_mobile = false;
    }
  ...
  MOBILE_OS_FAMILIES = expressions['mobile_user_agent_families'];
})(exports);